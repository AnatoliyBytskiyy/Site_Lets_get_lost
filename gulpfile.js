...
gulp.task('sass', function(){ // Создаем таск Sass
	return gulp.src('sources/sass/**/*.sass') // Берем источник
...
gulp.task('browser-sync', function() { // Создаем таск browser-sync
	browserSync({ // Выполняем browserSync
		server: { // Определяем параметры сервера
			baseDir: 'sources' // Директория для сервера - sources
		},
...
gulp.task('scripts', function() {
	return gulp.src([ // Берем все необходимые библиотеки
		...
		.pipe(gulp.dest('sources/js')); // Выгружаем в папку sources/js
});
...
gulp.task('watch', ['browser-sync', 'css-libs', 'scripts'], function() {
	gulp.watch('sources/sass/**/*.sass', ['sass']); // Наблюдение за sass файлами в папке sass
	gulp.watch('sources/*.html', browserSync.reload); // Наблюдение за HTML файлами в корне проекта
	gulp.watch('sources/js/**/*.js', browserSync.reload);   // Наблюдение за JS файлами в папке js
});
